...
const organismOptions: OrganismOption[] = [
  { key: 'sarpa', label: 'SARPA [SNAKE]', subOptions: [
    'A. DARVIKARA SARPA',
    'B. MANDALI SARPA',
    'C. RAJIMANTHA',
  ] },
  { key: 'vrischika', label: 'VRISCHIKA [SCORPION]', subOptions: [
  ...
  { key: 'loota', label: 'LOOTA [SPIDER]', subOptions: [
    '1. Karnika - Thorny',
    '2. Shopha - Oedema',
    '3. Jwara - Fever',
    '4. Kandu - Itching',
  ...
  { key: 'kita', label: 'KITA [INSECT BITE]', subOptions: [
    '1. Karnika - Thorny',
    '2. Shopha - Oedema',
    '3. Jwara - Fever',
    '4. Kandu - Itching',
  ...
  { key: 'vyala', label: 'VYALA SRRUGLA TRAKSHA RUKSHA VYAGHRI DASTA LAKSHANA (animal bites)', subOptions: [
    '1. Suptata - Numbness',
  ...
  { key: 'alarka', label: 'ALARKA [DOG]', subOptions: [
    '1. Suptah - Numbness',
  ...
  { key: 'mushika', label: 'MUSHIKA [RAT]', subOptions: [
      ...
      <div className="w-full max-w-2xl p-8 bg-white dark:bg-gray-900 rounded-2xl shadow-xl border border-indigo-200 dark:border-indigo-700 transition-colors duration-500">
        ...
        <div className="space-y-8">
          <div className="bg-indigo-50 dark:bg-gray-800 rounded-lg p-6 border border-indigo-100 dark:border-gray-700 transition-colors duration-500">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">I. BITE OF WHICH ORGANISMS</h2>
            <div className="grid gap-4">
              {organismOptions.map((opt) => (
                <div key={opt.key}>
                  <button
                    type="button"
                    className={`w-full text-left px-4 py-3 rounded-lg border transition font-medium text-base shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 dark:focus:ring-indigo-600 ${selectedOrganism === opt.key ? 'bg-indigo-600 text-white dark:bg-indigo-700' : 'bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-200 border-gray-300 dark:border-gray-700 hover:bg-indigo-100 dark:hover:bg-gray-700'}`}
                  ...
                  {selectedOrganism === opt.key && !opt.subOptions && (
                    <div className="mt-2 text-indigo-700 dark:text-indigo-300 text-sm">No sub-options.</div>
                  )}
                </div>
              ))}
            </div>
          ...
          <div className="bg-indigo-50 dark:bg-gray-800 rounded-lg p-6 border border-indigo-100 dark:border-gray-700 transition-colors duration-500">
            <label className="block text-sm font-medium text-gray-900 dark:text-gray-200 mb-2">II. TIME OF BITE</label>
            <input
              type="text"
              value={timeOfBite}
          ...
          <div className="bg-indigo-50 dark:bg-gray-800 rounded-lg p-6 border border-indigo-100 dark:border-gray-700 transition-colors duration-500">
            <label className="block text-sm font-medium text-gray-900 dark:text-gray-200 mb-2">III. SITE OF BITE</label>
            <input
              type="text"
              value={siteOfBite}
          ...
          <div className="bg-white dark:bg-gray-900 rounded-xl shadow-2xl max-w-md w-full p-6 relative animate-fadeIn">
            ...
            <div className="space-y-3 max-h-72 overflow-y-auto pr-2">
              {modalOption.subOptions?.map((sub) => (
                <button
                  key={sub}
                  className={`w-full text-left px-4 py-2 rounded-lg border transition font-medium text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 dark:focus:ring-indigo-600 ${selectedSarpa === sub ? 'bg-indigo-600 text-white dark:bg-indigo-700' : 'bg-indigo-50 dark:bg-gray-800 text-gray-900 dark:text-gray-200 border-gray-200 dark:border-gray-700 hover:bg-indigo-100 dark:hover:bg-gray-700'}`}
